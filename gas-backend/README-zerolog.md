...
**この節の予測は必ず実測で確認してから対処を決めること。** 対処の候補は
「`未学習日数` を `IFNULL(DATE_DIFF(CURRENT_DATE(), 最終学習日), 9999)` にして先頭へソートさせる」
「`リスク判定` の先頭に `WHEN 総解答数 = 0 THEN "⚫未着手"` を足す」
「表示は空欄のままにして、並べ替え専用の計算フィールドを別に足す」の3つ。
