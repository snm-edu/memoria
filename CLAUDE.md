...
2026-07-12 に GAS 側の High 課題（クライアント申告値の信頼・無認証の帰属書き換え・学籍番号単独照合・プロンプトインジェクション）へコード上の対策を実装した（サーバ側正誤再判定・studentId(UUID)必須化・問題バンク正本化・入力ホワイトリスト）。**GAS はコミットだけでは反映されない。次回 clasp push / エディタ更新でのデプロイが必要。**
