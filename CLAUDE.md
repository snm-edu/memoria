...
## セキュリティ対応状況（GAS バックエンド）

2026-04-18 監査でフロントエンド側（XSS/CSP/Zod/npm audit/DebugInfo）は対応済み。
2026-07-12 に GAS 側の High 課題（クライアント申告値の信頼・無認証の帰属書き換え・学籍番号単独照合・プロンプトインジェクション）へコード上の対策を実装した（サーバ側正誤再判定・studentId(UUID)必須化・問題バンク正本化・入力ホワイトリスト）。**本番反映は 2026-07-13 に完了済み**（2026-08-21 に本番プロジェクトを clasp clone して全ファイル差分を取り、`gas-backend/src/` と共通16ファイルが完全一致することを実測確認）。

残課題: エンドポイント全体の共通レート制限、AI呼び出しの per-student 日次上限（`MAX_AI_CALLS_PER_STUDENT_DAY` は定義のみ）。このリポジトリは Public のため、攻撃手順に直結する詳細はここには記載しない。詳細は非公開の課題管理（Claude Codeのプロジェクトメモリ）で追跡している。
